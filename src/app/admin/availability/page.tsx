...
export default function AdminAvailability() {
  ...
  const handleApproval = async (
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/doctor/${doctorId}/availability-slot/${slotId}`,
        {
          method: 'PATCH',
          headers: {
      ...
      <div className="space-y-6">
        {filteredDoctors.length === 0 ? (
          ...
          filteredDoctors.map((doctor) => (
            <div key={doctor._id} className="bg-white rounded-xl shadow-md border border-gray-100">
                      ...
                      <div className="flex-1">
                        ...
                        {slot.adminNotes && (
                          <div className="bg-white rounded-lg p-3 mt-2">
                            ...
                            <p className="text-sm text-gray-600">{slot.adminNotes}</p>
                          </div>
                        )}
                      </div>
                        ...
                        <div className="flex gap-2 ml-4">
                          <button
                            onClick={() => {
                              ...
                              handleApproval(doctor._id, slot.id, 'approved', notes || '');
                            }}
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (notes) {
                                handleApproval(doctor._id, slot.id, 'rejected', notes);
                              }
                            }}
