  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [appointmentsRes, doctorsRes, patientsRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/appointment`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/doctor`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/user`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      ...
      if (appointments.success) {
        const pending = appointments.data.filter((a: any) => a.status === 'pending');
        setStats({
          totalAppointments: appointments.data.length,
          pendingAppointments: pending.length,
          totalDoctors: doctors.success ? doctors.data.length : 0,
          totalPatients: patients.success ? patients.data.filter((u: any) => u.role === 'patient').length : 0,
        });
  ...
  return (
    <div>
      ...
      <div className="bg-gradient-to-r from-[#2952a1] to-[#1e3d7a] rounded-2xl p-8 text-white mb-8 shadow-lg">
        ...
        <p className="text-white/80">Manage your NovaHealth platform efficiently</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
          ...
          <h3 className="text-3xl font-bold text-gray-900 mb-1">{stats.pendingAppointments}</h3>
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <p className="text-[#2952a1] font-medium">{appointment.doctorName}</p>
                  </div>
