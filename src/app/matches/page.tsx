  ...
  const fetchMatches = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/patient/matches?${params}`,
        {
          headers: {
      ...
      if (result.success) {
        ...
        const doctorPromises = result.data.map((match: MatchResult) =>
          fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/doctor/${match.doctorId}`, {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#ebe2cd] via-white to-[#ebe2cd]/50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-600 font-medium">Finding your perfect matches...</p>
      ...
      <div className="min-h-screen bg-linear-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md text-center">
          ...
          <p className="text-gray-600 mb-6">
            We couldn't find any doctors matching your preferences. Try updating your preferences or search manually.
          </p>
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Your Doctor Matches</h1>
          <p className="text-xl text-gray-600">
            We found <span className="text-[#2952a1] font-semibold">{matches.length} doctor{matches.length !== 1 ? 's' : ''}</span> that perfectly match your preferences
          </p>
          {userLocation && (
        ...
        <div className="space-y-8">
          {matches.map((match) => {
            ...
            return (
              <div key={doctor._id} className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-300">
                <div className="p-8">
                  {/* Doctor Header */}
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center space-x-6">
                      ...
                      <div>
                        ...
                        <p className="text-lg text-gray-600 font-medium">{doctor.specialization}</p>
                        <div className="flex items-center mt-2 flex-wrap gap-2">
                          ...
                          {doctor.experience && (
                            <>
                              ...
                              <span className="text-sm text-gray-600 font-medium">{doctor.experience} years exp.</span>
                            </>
                          )}
                          {match.distance && (
                            <>
                              <span className="text-gray-400">•</span>
                              <span className="text-sm text-[#2952a1] font-medium">📍 {match.distance.toFixed(1)} km away</span>
                            </>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 mt-1">Perfect fit!</p>
                    </div>
                  </div>
                  ...
                  {/* Match Reasons */}
                  <div className="mb-6">
                    <h4 className="font-bold text-gray-900 mb-3">Why this is a great match:</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {match.matchReasons.map((reason, idx) => (
                        <div key={idx} className="flex items-center text-gray-700 bg-green-50 p-3 rounded-lg">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    ...
                    {doctor.languages?.map((lang) => (
                      <span key={lang} className="bg-[#ebe2cd] text-[#2952a1] px-3 py-1 rounded-full text-xs font-medium">
                        🗣️ {lang}
                      </span>
                    ))}
                  ...
                  {doctor.chamberLocation?.address && (
                    <div className="mb-6 p-4 bg-gray-50 rounded-xl">
                      <p className="text-sm font-semibold text-gray-700 mb-1">📍 Chamber Location:</p>
                      <p className="text-sm text-gray-600">{doctor.chamberLocation.address}</p>
        ...
        <div className="text-center mt-12">
          <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Need more options?</h3>
            <p className="text-gray-600 mb-6">Browse our complete directory of healthcare providers in your area</p>
            <button
              onClick={() => router.push('/search-doctors')}
