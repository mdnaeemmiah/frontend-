...
export default function VerifyEmailPage() {
  ...
  const onSubmit = async (data: VerifyFormData) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/auth/verify-email`, {
        method: 'POST',
        headers: {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/auth/resend-verification`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
