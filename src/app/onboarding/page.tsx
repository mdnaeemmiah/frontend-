  ...
  const checkExistingProfile = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/patient/profile`,
        {
          headers: {
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">
                  {isUpdating ? 'Update Your Preferences' : 'Find Your Perfect Doctor'}
                </h1>
                {isUpdating && (
          ...
          {step === 1 && (
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl font-bold mb-6 text-gray-900">What type of care do you need?</h2>
              ...
              <div>
                <h3 className="text-xl font-bold mb-4 text-gray-900">How would you like to receive care?</h3>
                <div className="space-y-3">
                  {[
                    { value: 'in-person', label: 'In-person visits only', desc: 'Traditional office visits' },
                    { value: 'virtual', label: 'Virtual appointments only', desc: 'Online consultations from home' },
                    { value: 'both', label: 'Both in-person and virtual', desc: 'Flexible care options' },
                  ].map((option) => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => setFormData({ ...formData, careMode: option.value })}
          ...
          {step === 2 && (
            <div className="space-y-8">
              <h2 className="text-2xl font-bold mb-6 text-gray-900">Where should we search?</h2>
              ...
              <div className="bg-[#ebe2cd]/50 border-2 border-[#2952a1]/30 rounded-2xl p-6">
                ...
                {formData.location.latitude === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-600 mb-6">We need your location to find nearby doctors</p>
                    <button
                      type="button"
                    ...
                    <p className="text-sm text-gray-500 mt-4">
                      We'll use your browser's location to find doctors near you
                    </p>
                  </div>
                ) : (
              ...
              {formData.location.latitude !== 0 && (
                <div className="bg-[#ebe2cd]/50 border-2 border-[#2952a1]/30 rounded-2xl p-6">
                  ...
                  <div className="text-center mb-4">
                    ...
                    <p className="text-sm text-gray-600 mt-1">We'll search within this distance</p>
                  </div>
                  <div className="space-y-3">
                    {[1, 2, 5, 10, 25, 50].map((radius) => (
                      ...
                      >
                        {radius} km {radius === 1 ? '(Very Close)' : radius === 2 ? '(Nearby)' : radius === 5 ? '(Local)' : radius === 10 ? '(City)' : radius === 25 ? '(Regional)' : '(Wide Area)'}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {step === 3 && (
            <div className="space-y-8">
              <h2 className="text-2xl font-bold mb-6 text-gray-900">What's your preferred communication style?</h2>
          ...
          {step === 4 && (
            <div className="space-y-8">
              <h2 className="text-2xl font-bold mb-6 text-gray-900">Additional Preferences (Optional)</h2>
              
              {/* Languages */}
              <div>
                <h3 className="font-bold text-gray-900 mb-3">Languages</h3>
                <div className="flex flex-wrap gap-3">
                  {['English', 'Spanish', 'French', 'Chinese', 'Hindi', 'Arabic'].map((lang) => (
                    <button
                      key={lang}
                      type="button"
                      onClick={() => setFormData({
                        ...formData,
                        languagePreferences: toggleArrayItem(formData.languagePreferences, lang)
                      })}
              ...
              {/* Vibe Preferences */}
              <div>
                <h3 className="font-bold text-gray-900 mb-3">Vibe Preferences</h3>
                <div className="flex flex-wrap gap-3">
                  {['lgbtq-affirming', 'family-friendly', 'holistic-approach', 'evidence-based', 'bilingual'].map((vibe) => (
                    <button
                      key={vibe}
                      type="button"
                      onClick={() => setFormData({
                        ...formData,
                        vibePreferences: toggleArrayItem(formData.vibePreferences, vibe)
                      })}
                      className={`px-4 py-2 rounded-xl font-medium transition-all ${
          ...
          <div className="flex justify-between mt-12">
            ...
            {step < totalSteps ? (
              <button
                ...
                onClick={nextStep}
                disabled={
                  (step === 1 && (!formData.careType || !formData.careMode)) ||
              ...
              >
                {loading ? 'Saving...' : isUpdating ? '✓ Update & Find Matches' : '✓ Find My Matches'}
              </button>
            )}
          </div>
        </div>
