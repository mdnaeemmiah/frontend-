...
const specializations = [
  ...
  'General Medicine', 'Psychiatry', 'Oncology', 'Endocrinology', 'Gastroenterology',
];

const vibeTags = [
  'warm-empathetic', 'direct-efficient', 'lgbtq-affirming', 'bilingual',
  'family-friendly', 'holistic-approach', 'evidence-based', 'patient-centered',
];

const languages = [
  'English', 'Spanish', 'French', 'German', 'Chinese',
  'Hindi', 'Arabic', 'Portuguese', 'Bengali',
];
...
export default function DoctorProfile() {
  ...
  const [formData, setFormData] = useState({
    ...
    languages: [] as string[], insuranceAccepted: [] as string[],
    vibeTags: [] as string[], communicationStyle: 'warm-empathetic' as any,
    consultationFee: '', telehealth: false, inPerson: false,
    acceptsNewPatients: true, introVideo: '',
  });
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({
          ...
          city: doctor.city || '', zipCode: doctor.zipCode || '',
          ...
          qualification: doctor.qualification || '', bio: doctor.bio || '',
          languages: doctor.languages || [], insuranceAccepted: doctor.insuranceAccepted || [],
          vibeTags: doctor.vibeTags || [],
          communicationStyle: doctor.communicationStyle || 'warm-empathetic',
          consultationFee: doctor.consultationFee?.toString() || '',
          telehealth: doctor.telehealth || false, inPerson: doctor.inPerson || false,
        ...
        if (doctor.availabilitySlots) setAvailabilitySlots(doctor.availabilitySlots);
        if (doctor.profileUpdateRequest) setProfileUpdateRequest(doctor.profileUpdateRequest);
      }
    } catch (error) {
  ...
  const handleVideoUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/doctor/my-intro-video`,
        {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API || 'http://localhost:5000'}/api/doctor/my-profile`,
        {
          method: 'PATCH',
          headers: {
  ...
  return (
    <div>
        ...
        >
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              >
                {profileUpdateRequest.status === 'pending'
                  ? `Submitted on ${new Date(profileUpdateRequest.requestedAt).toLocaleDateString()}. Waiting for admin approval.`
                  : profileUpdateRequest.status === 'approved'
                  ? `Approved on ${new Date(profileUpdateRequest.reviewedAt).toLocaleDateString()} by ${profileUpdateRequest.reviewedBy}`
                  : `Rejected on ${new Date(profileUpdateRequest.reviewedAt).toLocaleDateString()} by ${profileUpdateRequest.reviewedBy}`}
              </p>
              {profileUpdateRequest.adminNotes && (
                <div className="mt-3 p-3 bg-white rounded-lg">
                  <p className="text-sm font-semibold text-gray-700">Admin Notes:</p>
                  <p className="text-sm text-gray-600">{profileUpdateRequest.adminNotes}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Chamber Location */}
        <div className="bg-white rounded-xl shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Chamber Location 📍</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Google Maps URL</label>
              <input type="url" value={formData.chamberLocation.googleMapsUrl}
                onChange={(e) => handleGoogleMapsUrlChange(e.target.value)}
                placeholder="https://maps.google.com/..."
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" />
              {formData.chamberLocation.coordinates.coordinates[0] !== 0 && (
                <p className="mt-2 text-sm text-green-600">
                  ✓ Coordinates: {formData.chamberLocation.coordinates.coordinates[1].toFixed(6)}, {formData.chamberLocation.coordinates.coordinates[0].toFixed(6)}
                </p>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                ...
                <input type="text" value={formData.chamberLocation.address}
                  onChange={(e) => setFormData({ ...formData, chamberLocation: { ...formData.chamberLocation, address: e.target.value } })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">City</label>
                <input type="text" value={formData.chamberLocation.city}
                  onChange={(e) => setFormData({ ...formData, chamberLocation: { ...formData.chamberLocation, city: e.target.value } })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={formData.experience} onChange={(e) => setFormData({ ...formData, experience: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" min="0" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" value={formData.consultationFee} onChange={(e) => setFormData({ ...formData, consultationFee: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" min="0" step="0.01" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-gray-700 mb-2">Bio</label>
              <textarea value={formData.bio} onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
                rows={3} placeholder="Tell patients about yourself..."
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" />
            </div>
          </div>
        </div>
        ...
        {/* Intro Video */}
        <div className="bg-white rounded-xl shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Intro Video 🎥</h2>
          <div className="space-y-4">
            ...
            {videoPreview && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Current Video</label>
                <video src={videoPreview} controls className="w-full max-w-2xl bg-black rounded-lg">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button type="button" onClick={handleVideoUpload} disabled={!videoFile || uploadingVideo}
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-50 rounded-lg border border-green-200">
              <p className="text-sm text-green-800">
                📅 Add specific dates when you're available. Admin will approve your schedule.
              </p>
            </div>

            {/* Add New Slot */}
            <div className="p-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
              <h3 className="font-semibold text-gray-900 mb-3">Add New Availability</h3>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select value={newSlot.endTime} onChange={(e) => setNewSlot({ ...newSlot, endTime: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                    {timeSlots.map(time => <option key={time} value={time}>{time}</option>)}
                  </select>
                </div>
            ...
            {availabilitySlots.length > 0 && (
              <div className="space-y-3">
                <h3 className="font-semibold text-gray-900">Your Availability Slots</h3>
                {availabilitySlots.map((slot) => (
                  <div key={slot.id} className={`p-4 rounded-lg border-2 ${
                    slot.status === 'approved' ? 'border-green-500 bg-green-50' :
                    slot.status === 'rejected' ? 'border-red-500 bg-red-50' :
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                            slot.status === 'approved' ? 'bg-green-600 text-white' :
                            slot.status === 'rejected' ? 'bg-red-600 text-white' :
                        ...
                        {slot.adminNotes && (
                          <p className="text-sm text-gray-600 mt-2">
                            <span className="font-semibold">Admin Notes:</span> {slot.adminNotes}
                          </p>
                        )}
                      </div>
                      {slot.status === 'pending' && (
                        <button type="button" onClick={() => removeAvailabilitySlot(slot.id)}
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Languages & Tags</h2>
          <div className="space-y-4">
            <div>
                ...
                {languages.map(lang => (
                  <button key={lang} type="button"
                    ...
                    className={`px-3 py-1 rounded-lg font-medium transition-all ${
                      formData.languages.includes(lang) ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}>
                    {lang}
                  </button>
                ))}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Vibe Tags</label>
              <div className="flex flex-wrap gap-2">
                {vibeTags.map(tag => (
                  <button key={tag} type="button"
                    onClick={() => setFormData({ ...formData, vibeTags: toggleArrayItem(formData.vibeTags, tag) })}
                    className={`px-3 py-1 rounded-lg font-medium transition-all ${
                      formData.vibeTags.includes(tag) ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}>
                    {tag}
                  </button>
                ))}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-3">Communication Style</label>
              <div className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Care Options</label>
              <div className="space-y-2">
                <label className="flex items-center space-x-3 cursor-pointer">
                  <input type="checkbox" checked={formData.telehealth}
                    onChange={(e) => setFormData({ ...formData, telehealth: e.target.checked })}
                    className="w-5 h-5 text-green-600 rounded focus:ring-2 focus:ring-green-500" />
                  <span className="text-gray-700 font-medium">Offer Telehealth Appointments</span>
                </label>
                <label className="flex items-center space-x-3 cursor-pointer">
                  <input type="checkbox" checked={formData.inPerson}
                    onChange={(e) => setFormData({ ...formData, inPerson: e.target.checked })}
                    className="w-5 h-5 text-green-600 rounded focus:ring-2 focus:ring-green-500" />
                  <span className="text-gray-700 font-medium">Offer In-Person Appointments</span>
                </label>
                <label className="flex items-center space-x-3 cursor-pointer">
                  <input type="checkbox" checked={formData.acceptsNewPatients}
                    onChange={(e) => setFormData({ ...formData, acceptsNewPatients: e.target.checked })}
                    className="w-5 h-5 text-green-600 rounded focus:ring-2 focus:ring-green-500" />
                  <span className="text-gray-700 font-medium">Accepting New Patients</span>
                </label>
              </div>
            </div>
          </div>
